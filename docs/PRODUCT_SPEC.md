...
| **HD Clone** (RVC fine-tune)             | 10–30 minutes  | 30 min–2 hrs | ★★★★★ Excellent | Production calls        | **Implemented (M9, GPU fine-tune run pending)**        |

#### Training Pipeline (implemented — M9, GPU quality run pending)

Pipeline stages below are real (`gateway/app/training/tasks.py` drives them,
`inference/app/export/hd_train.py` runs feature_extraction through export). What is
**not** real yet: the actual fine-tune step and the pretrained HuBERT/F0/RVC weights it
needs — see [ROADMAP.md](ROADMAP.md)'s M9 section for exactly what that deferred GPU
run requires. Until then, Training/Export produce a contract-valid **synthetic**
stand-in model so the whole loop is exercised offline.
...
### VoiceModel (implemented — M9, GPU quality run pending)
...
class VoiceModel(SQLModel, table=True):
    id: UUID                                            # pk
    user_id: UUID                                       # owner (FK -> user.id, indexed)
    voice_id: UUID | None                                # FK -> voice.id (the source registry row)
    name: str                                           # Display name (e.g., "Morgan Freeman")
    type: VoiceModelType                                # instant | hd
    status: VoiceModelStatus                            # training | ready | failed

    # Training metadata — driven by the Celery task (app/training/tasks.py) as
    # the pipeline advances; polled by GET /api/voices/:id/train/status.
    ...
    # Model artifacts — model_path is the exported model's id, i.e. the
    # self_hosted streaming model_id ({model_path}.onnx in SELF_HOSTED_MODEL_DIR).
    model_path: str
    model_size_bytes: int

    # Quality metrics — no automated scorer exists yet (v1 is a listening
    # check, §15); unset unless filled in by hand.
...
### HD Engine: RVC (Retrieval-based Voice Conversion — implemented M9, GPU fine-tune pending)
...
**What M9 shipped:** the three-stage composition above really does export to a
single audio-in/audio-out ONNX graph satisfying the M5a streaming contract
(`inference/app/export/rvc/compose.py`) — proving the architectural risk this
milestone was flagged for. What it does **not** ship: real pretrained
HuBERT/F0/RVC weights or an actual per-speaker fine-tune loop (no FAISS index
either) — the composed graph runs on placeholder weights, and the training API
(`POST /api/voices/:id/train`, Celery pipeline) produces a **contract-valid
synthetic stand-in** model rather than a real fine-tune whenever no real
export exists, which today is always. Vendoring real weights and running the
fine-tune on a rented GPU is the deferred tail (ROADMAP.md's M9 section);
until then, self-hosted OpenVoice remains the primary, quality-proven engine.
...
| 6   | **Engineering health**: CI green on `main` (format + lint + tests × 3 services + image builds)                                                                                          | GitHub Actions (M7)                                                                                                  | **Partial** — gateway (83) and inference (79, 1 skipped) test suites pass unchanged; M10's `/` → Dashboard move makes 2 pre-existing frontend assertions in `test_pages.py` stale (they assert Monitor content at `/`) until the test-author pass updates them for the new route |
